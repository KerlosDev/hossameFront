    ...
    const fetchEnrolledCourses = async () => {
        ...
        if (isCacheValid) {
            try {
                ...
                // Filter out courses with null courseId or missing data from cache
                const validCourses = parsedCache.filter(course =>
                    course &&
                    course.courseId &&
                    course.courseId._id &&
    ...
    const fetchFromAPI = async (showLoading = true) => {
        ...
        try {
            ...
            if (response.data.isHeEnrolled && response.data.coursesAreEnrolled?.length > 0) {
                // Filter out courses with null courseId or missing course data
                const validCourses = response.data.coursesAreEnrolled.filter(course =>
                    course &&
                    course.courseId &&
                    course.courseId._id &&
                    course.courseId.name
            ...
            {enrolledCourses.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {enrolledCourses.map((course) => {
                        ...
                        if (!course?.courseId || !course.courseId.name) {
                            return null; // Skip courses without proper courseId data
                        }
                                    ...
                                    <div className="relative flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-xl font-arabicUI2 text-white group-hover:text-blue-300 transition-colors duration-300">
                                                {course.courseId.name || 'اسم الكورس غير متاح'}
                                            </h3>
                                            <p className="text-blue-100 text-sm">{course.courseId.level || 'المستوى غير محدد'}</p>
                                        </div>
                                    </div>
                                </div>

                                <div className="p-6">
                                    <p className="text-gray-700 dark:text-white/80 mb-4 line-clamp-2">{course.courseId.description || 'لا يوجد وصف متاح للكورس'}</p>
